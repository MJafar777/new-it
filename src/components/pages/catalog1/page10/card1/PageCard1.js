...
import "./pageCard1.css";
import imgHom from "../../images/hoover-main.jpeg";
  ...
  return (
    <Box className="catalog1-page1-card1-container">
      <Box className="catalog1-page1-card1-container-grid1">
        ...
        component="div"
        sx={{marginY: 4}}
        >Сервисный центр → Ремонт ноутбуков</Typography>
        ...
        sx={{marginY: 4}}
        >
          Мастер с опытом 15 лет починит ваш пылесос за пару часов!
        </Typography>
      </Box>
      ...
      <Box  className="catalog1-page1-card1-container-grid1">
          ...
          {categoty.map((item, i) => {
            return (
              <Card key={i} sx={{ width: 170, boxShadow: 0, background: 'none', display:'flex' }}>
                ...
                <Typography sx={{ marginLeft: 2, fontWeight: "500", textAlign: 'left' }}>
                  {item}
                </Typography>
              </Card>
            );
        ...
        <Typography variant="p">Менеджер перезвонит Вам в течение 30 секунд</Typography>
      </Box>
    </Box>
  );
}
